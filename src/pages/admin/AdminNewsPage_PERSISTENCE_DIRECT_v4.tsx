...
const NEWS_FORM_STORAGE_KEY = 'admin:news:form';
const NEWS_EDITING_ID_STORAGE_KEY = 'admin:news:editingId';
const NEWS_SHOW_FORM_STORAGE_KEY = 'admin:news:showForm';
...
function writeSessionStorage<T>(key: string, value: T) {
  ...
  try {
    ...
    // Persistência temporária é best-effort e não deve bloquear o formulário.
  }
}
...
export function AdminNewsPage() {
  ...
  const [form, setForm] = useState(() =>
    readSessionStorage(NEWS_FORM_STORAGE_KEY, initialForm),
  );
  const [editingId, setEditingId] = useState<string | null>(() =>
    readSessionStorage<string | null>(NEWS_EDITING_ID_STORAGE_KEY, null),
  );

  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [showForm, setShowForm] = useState(() =>
    readSessionStorage(NEWS_SHOW_FORM_STORAGE_KEY, false),
  );
  ...
  useEffect(() => {
    writeSessionStorage(NEWS_FORM_STORAGE_KEY, form);
  }, [form]);

  useEffect(() => {
    writeSessionStorage(NEWS_EDITING_ID_STORAGE_KEY, editingId);
  }, [editingId]);

  useEffect(() => {
    writeSessionStorage(NEWS_SHOW_FORM_STORAGE_KEY, showForm);
  }, [showForm]);
  ...
  function handleChange(
    ...
    setForm((currentForm) => {
      ...
      // Grava no mesmo evento do input. Assim a navegação imediata não depende
      // de um useEffect posterior para persistir o formulário.
      writeSessionStorage(NEWS_FORM_STORAGE_KEY, nextForm);
      return nextForm;
    });
  }
  ...
  function resetForm() {
    ...
    window.sessionStorage.removeItem(NEWS_FORM_STORAGE_KEY);
    window.sessionStorage.removeItem(NEWS_EDITING_ID_STORAGE_KEY);
    window.sessionStorage.removeItem(NEWS_SHOW_FORM_STORAGE_KEY);
  }
  ...
  function handleEdit(item: GdrbNews) {
    ...
    writeSessionStorage(NEWS_EDITING_ID_STORAGE_KEY, item.id);
    writeSessionStorage(NEWS_FORM_STORAGE_KEY, nextForm);
    writeSessionStorage(NEWS_SHOW_FORM_STORAGE_KEY, true);
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              onClick={() => {
                ...
                if (nextShowForm) {
                  writeSessionStorage(NEWS_EDITING_ID_STORAGE_KEY, null);
                  writeSessionStorage(NEWS_FORM_STORAGE_KEY, initialForm);
                  writeSessionStorage(NEWS_SHOW_FORM_STORAGE_KEY, true);
                } else {
                  window.sessionStorage.removeItem(NEWS_FORM_STORAGE_KEY);
                  window.sessionStorage.removeItem(NEWS_EDITING_ID_STORAGE_KEY);
