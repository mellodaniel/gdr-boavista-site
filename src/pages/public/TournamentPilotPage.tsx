...
const steps: Array<{ id: PilotStep; title: string; description: string }> = [
  ...
  },
  {
    id: 'summary',
    title: 'Resumo',
    description: 'Validação e criação do torneio piloto.',
  },
];
...
function getValidationMessages(form: PilotForm) {
  ...
  if (form.createResultsAccess && !form.resultsUsername.trim()) {
    messages.push({ type: 'warning', text: 'Indicou acesso de resultados, mas ainda não definiu o nome do utilizador.' });
  }
...
export function TournamentPilotPage() {
  ...
  const scheduledMatches = useMemo(() => schedulePairings(form), [form]);
  ...
  async function getUniqueSlug(baseValue: string) {
    const baseSlug = createSlug(baseValue || 'torneio-piloto');
  ...
  async function handleCreateTournament(event: FormEvent<HTMLFormElement>) {
    ...
    if (hasBlockingErrors) {
      setErrorMessage('Corrige os erros de validação antes de criar o torneio piloto.');
      return;
    }
    ...
    try {
      ...
      if (!sessionData.session) {
        throw new Error('Para criar o torneio piloto é necessário estar autenticado no admin.');
      }
      ...
      const { data: tournament, error: tournamentError } = await supabase
        ...
        .insert({
          ...
          description:
            ...
            'Torneio criado através do assistente piloto de criação self-service.',
      ...
      const dayRows = getDaysDraft(form).map((day) => ({
        ...
        notes: form.hasLunchBreak ? 'Criado pelo assistente piloto com pausa configurada.' : 'Criado pelo assistente piloto.',
      }));
      ...
      const { data: createdFields, error: fieldsError } = await supabase
        ...
        .insert(
          fields.map((field) => ({
            ...
            surface: null,
            is_active: true,
            notes: 'Campo criado pelo assistente piloto.',
          })),
        )
        .select('id, name');
      ...
      const { data: createdTeams, error: teamsError } = await supabase
        ...
        .insert(
          teams.map((team) => ({
            ...
            notes: 'Equipa placeholder criada pelo assistente piloto. Editar depois no admin.',
      ...
      const { error: rulesError } = await supabase.from('tournament_rules').insert({
        ...
        notes: `Criado pelo assistente piloto. Acesso de resultados solicitado: ${form.createResultsAccess ? 'sim' : 'não'}${
      ...
      const matchRows = schedulerResult.matches.map((match) => ({
        ...
        score_a: null,
        score_b: null,
        notes: match.notes || 'Jogo gerado pelo assistente piloto.',
      }));
      ...
      setSuccessMessage('Torneio piloto criado com sucesso. Foram gerados os dados iniciais, página pública, jogos da fase de grupos e placeholders da fase final.');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Não foi possível criar o torneio piloto.';
          ...
          <div className="grid gap-8 p-8 md:grid-cols-[1.4fr_0.8fr] md:p-12">
            <div>
              ...
              <h1 className="mt-4 font-serif text-4xl font-light leading-tight md:text-6xl">
                Criar torneio com assistente.
              </h1>
              <p className="mt-5 max-w-3xl text-sm leading-7 text-white/75 md:text-base">
                Responde a perguntas simples e o sistema cria a estrutura inicial do torneio:
            ...
            <div className="rounded-2xl bg-white/10 p-6 ring-1 ring-white/10">
              ...
              <h2 className="mt-4 text-xl font-bold">O que será criado</h2>
              <ul className="mt-4 space-y-3 text-sm leading-6 text-white/75">
                <li>• Torneio publicado com link público</li>
                <li>• Dias, horários e campos</li>
                <li>• Equipas placeholder e grupos</li>
                <li>• Parceiro próprio do torneio, se existir</li>
                <li>• Jogos da fase de grupos</li>
                <li>• Link admin para lançar resultados</li>
              </ul>
            </div>
          </div>
        </section>
          ...
          <form onSubmit={handleCreateTournament} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
              ...
              >
                <div className="space-y-5">
                    ...
                    <div className="grid gap-5 md:grid-cols-2">
                      ...
                      <Field label="Palavra-passe temporária">
                        <input
                          ...
                          placeholder="Não será gravada nesta versão piloto"
                        />
                      </Field>
                    </div>
                  )}
                  ...
                  <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
                    Nesta versão, o assistente cria o torneio e mostra o link de gestão de jogos.
                    A criação automática do utilizador no Supabase deverá ser feita numa fase seguinte com Edge Function segura.
                  </div>
                </div>
              </StepCard>
            )}
            ...
            {currentStep === 'summary' && (
              <StepCard
                ...
                description="Confirma os dados antes de criar o torneio real como piloto."
              >
                <div className="space-y-6">
                  ...
                  <div className="rounded-2xl bg-[#24180f] p-5 text-sm leading-6 text-white">
                    <p className="font-bold">Script de criação previsto</p>
                    <pre className="mt-3 overflow-x-auto whitespace-pre-wrap text-xs text-white/75">
{JSON.stringify(
  {
    createTournament: true,
    publishPublicPage: true,
    createDays: dates.length,
    createFields: fields.length,
    createPlaceholderTeams: teams.length,
    createGroups: groups.length,
    createRules: true,
    createTournamentPartner: form.hasTournamentPartner,
    generateGroupMatches: scheduledMatches.scheduled.length,
    unscheduledGroupMatches: scheduledMatches.unscheduled.length,
    createResultsAccessRequest: form.createResultsAccess,
  },
  null,
  2,
)}
                    ...
                    <div className="rounded-2xl border border-green-200 bg-white p-5 shadow-sm">
                      ...
                      {form.createResultsAccess && (
                        <div className="mt-4 rounded-xl bg-amber-50 p-4 text-sm leading-6 text-amber-900">
                          Acesso de resultados solicitado: <strong>{form.resultsUsername || 'por definir'}</strong>.
                          Cria/associa este utilizador no Supabase/Admin antes de entregar o acesso.
                        </div>
                      )}
                    </div>
                  )}
              ...
              <div className="flex flex-col-reverse gap-3 md:flex-row md:justify-end">
                ...
                {currentStep !== 'summary' ? (
                  ...
                  >
                    ...
                    {isCreating ? 'A criar torneio...' : 'Criar torneio piloto real'}
                  </button>
                )}
              </div>
            </div>
