...
function isDateTodayOrFuture(dateValue: string) {
  const date = new Date(`${dateValue}T12:00:00`);
  return date >= startOfToday();
}

function isRecentResultForHomepage(dateValue: string) {
  const matchDate = new Date(`${dateValue}T12:00:00`);
...
export function HomePage() {
    ...
    async function loadHomeData() {
      ...
      const [
        ...
        supabase
          ...
          .select('*')
          .eq('is_visible', true)
          .in('status', ['agendado', 'adiado', 'cancelado', 'terminado'])
  ...
  const agendaItems = useMemo<AgendaItem[]>(() => {
    const upcomingMatches: AgendaItem[] = matches
      .filter(
        (match) =>
          match.is_visible &&
          ['agendado', 'adiado', 'cancelado'].includes(match.status) &&
          isDateTodayOrFuture(match.match_date),
      )
      .map((match) => ({
        type: 'match',
        id: match.id,
    ...
    const upcomingTournaments: AgendaItem[] = tournaments
      .filter(
        (tournament) =>
          ...
          isDateTodayOrFuture(tournament.end_date || tournament.start_date),
      )
      .map((tournament) => ({
        type: 'tournament',
        id: tournament.id,
    ...
    return matches
      .filter(
        (match) =>
          ...
          isRecentResultForHomepage(match.match_date),
      )
      .sort((a, b) =>
      ...
      <section className="bg-[#24180f] py-24 text-white">
        <div className="mx-auto max-w-7xl px-4">
          ...
          {recentResults.length === 0 ? (
            <div className="mt-10 rounded-sm border border-white/10 bg-white/5 p-10 text-center text-zinc-300">
              <ShieldCheck className="mx-auto text-red-400" size={34} />

              <h3 className="mt-5 font-serif text-3xl font-light text-white">
            ...
            <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {recentResults.map((match) => {
                ...
                return (
                  <article
                    key={match.id}
                    className="overflow-hidden rounded-sm border border-white/10 bg-white text-[#24180f] shadow-xl shadow-black/10"
                  >
                    <div className="h-1.5 bg-red-700" />
                    ...
                    <div className="p-7">
                      ...
                      <div className="mt-6 grid gap-3 text-center">
                        ...
                        <div className="rounded-sm bg-[#24180f] px-5 py-4 text-4xl font-black text-white">
